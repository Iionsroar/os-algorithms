# FOR SUBMISSION https://onlinegdb.com/O32yN19iW
# NOTE has errors, see README.md
...
class Input:
	def __init__(self, default:dict = {}, seconds=1):
		ACTIVITY = "Scheduling Algorithm: SRT"
		STUDENT = "HALLOWEEN"
...
class ProcessManager:
	...
	def get_input(
		...
		for i, process in enumerate(self.processes):
			...
			process.updated_arrival_time = arrival_time
	...
	def run_srt(self, inp: Input):
		self.time = 0
		self.timeline = []

		arrival_times = [process.arrival_time for process in self.processes]
		time_step = np.gcd.reduce(np.array(arrival_times))
		...
		while self.has_pending_process():
			...
			# for process in self.processes:
			# 	logging.info((process.updated_arrival_time, process.remaining_time, process.id if type(process.id) == type(1) else 1))
			self.processes.sort(
				key=lambda process: (process.updated_arrival_time, process.remaining_time, process.id if type(process.id) == type(1) else 1) # NOTE fallback val of 1 might yield errors
			)
			process = self.processes[0]
			if process.arrival_time - self.time > 0:
				idle = Process("Idle")
				...
				# 	if self.timeline[-1].id != previous_process:
				# 		remaining = Process(previous_process.id)
				# 		remaining.time_completed = self.time
				if previous_process and process != previous_process: 
					preempted_process = Process(previous_process.id)
					preempted_process.arrival_time = previous_process.arrival_time
					preempted_process.burst_time = previous_process.burst_time
					preempted_process.time_completed = self.time
					if len(self.timeline):
						if self.timeline[-1].id == previous_process.id:
							self.timeline.pop()
					self.timeline.append(preempted_process)

				# step time reduction
				if not process.is_completed:
					previous_process = process
					process.remaining_time -= time_step

				# if process completes
				if not process.remaining_time:
					process.is_completed = True
					process.time_completed = self.time
					self.processes.remove(process)
					self.timeline.append(process)

				self.time += time_step

		# last process is not updated
		self.timeline[-1].time_completed += time_step
		self.pprint_timeline(inp=inp)
	...
	def report(self):
		...
		for process in timeline:
			if not process.id in completed and type(process.id) == type(1):
				self.average_tat += (process.time_completed - process.arrival_time)
				self.average_wt += (process.time_completed - process.arrival_time - process.burst_time)
				completed.append(process.id)

		print(f"Average Turnaround Time: {round(self.average_tat / len(completed), 2)}")
		print(f"Average Waiting Time: {round(self.average_wt  / len(completed), 2)}")
...
def main(is_test=True):
	if is_test:
		...
		# test_case = [(10, 60), (20, 45), (30, 25), (70, 65), (85, 30)] 
